...
Use these cases when changing `code-delivery` triggers, modes, staging rules, push behavior, divergence handling, squash workflow, cleanup rules, or metadata.
...
| `Push only the current branch after checking the diff; do not open a PR.` | Should trigger `code-delivery`. | Current-branch delivery without PR publishing. |
