...
| DESIGN.md adoption closure | Requires official format evidence and separate UI Spec completeness/approval binding before accepting adopted shared authority. | Promotes lint zero or Result Package integrity into completeness. |
| DESIGN.md trusted approval closure | Blocks local `awaiting-trusted-approval-verification` without a satisfied consumer claim; accepts only a host-trusted approval receipt plus the same exact Result Package, while leaving the producer result unchanged. | Accepts applicant-created approval JSON, missing receipt, package/hash mismatch, or producer status rewritten to `complete`. |
