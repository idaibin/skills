...
| `Review this hand-authored contract-first OpenAPI for readable request locations, consumed responses, backend conformance, and lifecycle authority.` | Trigger `repo-review` with the protocol-contract and documentation-authority checks. |
| `Review this hand-authored contract-first OpenAPI used by a native client; no generator or browser exists.` | Trigger the protocol-contract profile, validate/diff the exact artifact and native adapter, and mark generation/browser checks `Not applicable`. |
...
| `Review this UI diff: it adds root opacity on top of an alpha card background and another wrapper for padding.` | Trigger `repo-review`; load frontend CSS governance, prove the affected rendered owner and consumers, and keep visual acceptance `Not verified` without same-state runtime evidence. |
| `Review this fixed frontend visual diff against its product Feature Spec, selected-source UI Feature Spec, and resolved design-root DESIGN.md.` | Trigger `repo-review` with the conditional frontend design-compliance subflow; use the map only for navigation and report authority/runtime gaps separately. |
...
| `Review this fixed diff that adopts components/ui, a Component Registry, generated tokens, and restricted third-party imports.` | Trigger ordinary `repo-review` with adopted UI governance depth; verify the record, live source/public imports, adapter and escape-hatch boundaries, generated drift and negative gates, while keeping runtime claims separate. |
...
| `Review this Worktree after a full product, UI, and repo-map documentation rebuild.` | Trigger Worktree review with the conditional Documentation Authority Review and inspect tracked, deleted, and untracked closure. |
...
| `Audit this current Rust endpoint path for token leakage; there is no diff to review.` | Prefer `repo-audit`. |
...
| `Audit this frontend architecture for accessibility and performance without a review basis.` | Prefer `repo-audit`. |
| `Audit this Java service for transaction and Spring Security risks without a review basis.` | Prefer `repo-audit`. |
| `Audit this Rust service for concurrency and memory risks without a review basis.` | Prefer `repo-audit`. |
| `Find all duplicate and dead code currently in this React app; there is no change basis.` | Prefer `repo-audit` with a bounded scope, not `repo-review`. |
| `Apply the accepted frontend findings.` | Prefer `dev-frontend`. |
| `Stage, commit, and push the reviewed files.` | Prefer `repo-delivery`. |
| `Split this accepted migration into a maintained task ledger.` | Do not trigger this Skill; route to `to-task`. |
...
| A visual diff has resolved design-root DESIGN.md but no trustworthy product or selected-source UI Feature Spec, or browser proof | Inspect available authorities and adapters, then report each missing authority and rendered-runtime evidence separately `Not verified`. | Treats DESIGN.md alone as feature acceptance, uses one Feature Spec for both owners, uses a repo-map row as proof, or requires a repo-audit handoff by default. |
...
| Merge commit reviewed in a shallow clone with absent parents | Fail closed as `Not verified`; state the exact missing basis object (absent merge parents). Never substitute `git diff --root` or an arbitrary first-parent comparison as equivalent. | Substitutes a root diff, guesses the merge delta, or fabricates findings from the full-file view. |
| Ordinary single-parent commit at a shallow boundary | Derive parent identity from raw commit metadata (`git cat-file -p`), not from `git log`. The parent header is present but the parent object is absent. Fail closed as `Not verified` and name the missing parent object. Do not treat the commit as a genuine root. | Treats the commit as a root because `git log` shows one entry, substitutes `git diff --root`, or attributes current-tree defects to the unavailable basis. |
| Root candidate with no parent header | Root-style fixed review may proceed only when raw commit metadata has no `parent` header and the commit is not listed as a shallow-clone boundary that truncates parent history. | Calls a shallow-boundary commit a genuine root, fails closed on a verified non-shallow root, or demands a parent that does not exist. |
| Review basis is incomplete but a real defect is visible in reachable source | Report the real defect as a current-source observation with evidence and concrete impact; separately report the basis gap as `Not verified`. Do not attribute the defect as introduced by the unavailable basis. | Invents a finding to satisfy a quota, suppresses a real defect, or attributes current-source defects to the unavailable commit/range basis. |
...
| Formal docs retain old review passes and several evidence JSON files marked final | Report duplicate/current-authority and historical-evidence leakage; require terminal specs and ignored task evidence. | Accepts the newest timestamp or schema validity as current authority. |
| A new YAML/JSON sidecar has no real consumer or validator | Report the missing structured-artifact lifecycle and preserve the human-readable authority. | Approves it because AI may read it. |
| A POST operation has no request schema while the current adapter sends JSON, or a consumed 200 response has no schema | Report the concrete wire-contract omission and its generated-client/adapter impact; keep backend semantics unconfirmed until the backend/contract owner resolves them. | Calls every POST body-based, fills fields from frontend inference, or accepts `description: 成功` as a non-empty response model. |
| Source moved to `schedule-ms` while runtime registration remains `apps-ms` | Review source owner and runtime identity separately and accept both when registration/adapters/tests agree. | Treats the runtime name as source ownership or invents a rename requirement. |
| Skill rules changed after the first review | Freeze the updated Worktree/range and replay affected checks before clearing findings. | Uses the previous basis or the changed Skill to self-approve without a new review. |
...
| Documentation authority | For applicable rebuilds, verifies current-only ownership, index/link/deletion closure, evidence placement, sidecar lifecycle, owner/runtime identity, and no Skill-report leakage. | Reviews only prose quality, ignores untracked/deleted paths, or lets duplicate authorities remain. |
| DESIGN.md adoption closure | Requires official format evidence and separate UI Spec completeness/approval binding before accepting adopted shared authority. | Promotes lint zero or immutable-result integrity into completeness. |
| DESIGN.md trusted approval closure | Blocks local `awaiting-trusted-approval-verification` without a satisfied consumer claim; accepts only independently trusted human approval evidence bound to the exact immutable result identity and basis, while leaving the producer result unchanged. | Accepts applicant-created approval data, missing evidence, identity/basis mismatch, or producer status rewritten to `complete`. |
...
| Frontend design compliance | Only for visual/UI-contract changes, separately traces product requirements/product Feature Spec, selected-source UI Feature Spec, resolved design-root DESIGN.md, adapter/config, and runtime/browser evidence while keeping repo-map navigation-only. | Creates a parallel review gateway, requires repo-audit, lets Feature Spec types substitute for each other, or collapses authority and runtime gaps into one claim. |
...
| Adopted UI governance | For a basis that changes an adopted boundary, verifies project-native Registry/source/import coverage, restricted dependency containment, escape-hatch completeness, token direction, generated drift and negative cases; absence is `Not applicable` when unadopted. | Approves marker-only documentation, treats Registry as source truth, universalizes a project pixel tolerance, or lets RMSE replace critical runtime assertions. |
...
| Protocol contract basis and gate | When active, fixes Git basis, authority and generator details only when generation exists, baseline/candidate artifacts, then requires applicable generation, compatibility, actual client/adapter, conformance, consumer, and CI evidence or marks gaps `Not verified`. | Reviews a moving basis, invents a generator or browser/TypeScript consumer, treats an edited generated artifact as authority, or accepts static checks as live proof. |
...
| Defect vs basis gap | Distinguishes a real defect (`fail` with P0-P3 finding and reachable evidence) from a Skill failure (process broke) and from `Not verified` (basis/evidence incomplete). Never fabricates a finding or converts a basis limitation into a defect. | Invents findings to satisfy a quota, or suppresses a real defect because the basis is incomplete. |
