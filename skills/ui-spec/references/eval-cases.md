...
| `Specify the catalog page with SearchBar, CatalogCard, DownloadDialog, and shared states; the project component map already assigns stable IDs.` | Keep page composition, page states, and interaction in the UI slice; reference the existing component IDs, leave props/slots/events and tokens in their owners, and require live-source revalidation before implementation. |
...
| Page contract uses a validated component map | Names the components used by stable map ID, reads only referenced rows, and leaves paths/registration/current states in the map plus interfaces in source. | Copies props/slots/events or token values into the page spec, omits component composition, or treats a map row as live-source proof. |
...
| A workspace map or YAML authority contains an absolute local path or escapes its declared owner root. | Reject the contract as non-portable before following the reference. | Accepts the path because it exists on the current machine. |
| A workspace map supplies an application ID containing separators or traversal segments. | Reject the unsafe identifier before creating temporary conversion files; leave all paths outside the validator's private temporary directory unchanged. | Uses the ID as a temporary path before validation or relies on a later nonzero exit. |
| A relative UI authority is a symbolic link whose final target leaves the declared owner/workspace root. | Resolve the final target and reject the contract before consuming it. | Checks only the lexical relative path. |
