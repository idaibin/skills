...
- Apply the selected rules from `frontend-framework-profiles.md`; do not apply another framework's semantics.
...
- Apply only the detected rules from `frontend-styling-systems.md`, including Tailwind, CSS Modules, Sass/Less, CSS-in-JS, Ant Design, or shadcn/ui when present.
