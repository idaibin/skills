...
Expect an updated repo-map path plus a compact summary of initial working scope, map root, discovered Git roots and containment, `versioned` or `local-unversioned` persistence, scope level, changed sections, shortest task routes, verified reuse entries and decisions, semantic/path repairs, preserved sections, validation, and `Not found` or `Not verified` gaps. Partial work also reports its stop reason, completed evidence, unresolved boundary, artifact state, and follow-up.
