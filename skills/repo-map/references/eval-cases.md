...
| Frontend design binding | For a reusable UI row, records the exact root `DESIGN.md` relative path plus anchor/semantic binding, adapter/config, implementation/export/consumer, states/variants, and source evidence. | Copies shared token values/rules, treats DESIGN.md as implementation proof, or makes the inventory mandatory. |
