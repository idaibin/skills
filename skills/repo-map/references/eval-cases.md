...
| `Give the selected frontend components stable IDs so page UI contracts can reference them, but keep interfaces in source and shared visual semantics in DESIGN.md.` | Trigger the Frontend Inventory profile for a bounded component map. | Durable page-to-owner navigation is requested without creating a parallel component contract. |
...
| Page component reference | Uses stable owner-scoped IDs for selected components, lets page specs reference them, keeps interfaces in source, names inline regions honestly, and separates current variants from target-only gaps. | Copies props/slots/events, invents importable symbols, stores page composition in the map, or presents unimplemented states as current. |
| A page contract links to a structured component map used by several consumers. | Keep the map owner-local and navigation-only; require lifecycle metadata for machine consumption, preserve source as proof, and keep backend DTO/OpenAPI authority external. | Publishes a schema-less map as authority, copies API schemas or visual tokens, or treats a map row as runtime/source proof. |
...
| A workspace or owner-local map records an absolute machine path or `../` escape. | Reject it as non-portable even when the path exists on the current host. | Treats local existence as valid map ownership. |
| `structure.json`, `routes.json`, `components.json`, or `api.json` is truncated to owner-only or loses a required collection. | Fail the frontend map profile on missing/wrong-type/empty required fields; a legitimate empty API inventory uses an explicit `not-applicable` status. | Treats every missing collection as an empty valid list. |
| A relative map path is a symbolic link whose final target leaves the owner/workspace root. | Resolve the final target and reject the entry. | Validates only `path.resolve` lexical containment. |
