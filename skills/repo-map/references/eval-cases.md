...
| `Create docs/project-map.md from the repository's current truth.` | Trigger Repo Map mode. | Initial durable map. |
...
| `Update the root map, keep the specialist Markdown component navigation, and route token semantics only to DESIGN.md.` | Trigger Targeted Update with federated specialist-map routing. | The root may route to bounded Markdown navigation, while DESIGN.md remains the only visual-token authority. |
...
| `Map the high-value source owners for the page's named component roles without copying props, states, or page composition.` | Trigger the Frontend Inventory profile for bounded Markdown navigation. | Durable page-to-owner navigation is requested without creating a parallel component registry. |
...
| Page component navigation | Keeps page composition in UI Markdown, links only high-value source owners, names inline regions honestly, and separates current implementation from target gaps. | Invents a component-ID registry, copies props/slots/events, stores page composition in the map, or presents unimplemented states as current. |
| A page contract links to an existing structured component registry used by named non-LLM consumers. | Keep the registry owner-local and navigation-only; require lifecycle metadata, preserve source as proof, and keep backend DTO/OpenAPI authority external. | Creates a registry solely for AI, copies API schemas or visual tokens, or treats a registry row as runtime/source proof. |
...
| Structured map admission | Adds a sidecar only with a named owner, producer, non-LLM consumer, semantic version, executable validator, drift policy, and retirement rule. | Creates `map.yaml` or a schema merely for AI parsing. |
| A project map uses an absolute machine path or escapes its owner root. | Reject it as non-portable and replace it with a verified owner-relative path. | Treats current-machine existence as durable ownership. |
| A project map uses an owner-relative symbolic link whose final target leaves the owner/workspace root. | Resolve the final target and reject the entry without requiring a structured map schema. | Validates only lexical containment. |
| An AI-only component/token sidecar has no named non-LLM consumer or complete lifecycle. | Reject the sidecar; retain specialist Markdown navigation and route token semantics to `DESIGN.md`. | Keeps the sidecar because YAML/JSON is easier for an LLM to parse. |
