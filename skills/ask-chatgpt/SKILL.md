...
- Keep outbound `<review-id>-package.md` separate from inbound local-private `<review-id>-response.md` inside the ignored `.codex/reviews/` parent. A repository-delivered review is a separately authorized, sanitized durable copy, not the raw workspace.
