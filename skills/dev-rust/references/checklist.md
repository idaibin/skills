...
For HTTP REST, select the Protocol-contract overlay only when current source/CI
already owns that pipeline or the task explicitly requests a migration/trial.
Otherwise keep the native route/DTO/client/test boundary and mark OpenAPI
`Not applicable`. When active, record one code-first or contract-first authority.
...
- For HTTP REST, trace the native authority -> client -> representative consumer.
  Add normalized OpenAPI and generated-client steps only when the protocol overlay
  is active; use actual commands and never assume a framework/tool.
...
- Static schema checks do not satisfy the live protocol gate. Mark unavailable
  generation, compatibility, runtime conformance, consumer, or CI evidence
  `Not verified`.
