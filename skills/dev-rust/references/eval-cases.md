...
| Protocol automation: `Extend the existing OpenAPI pipeline with one authenticated operation.` | Baseline + Protocol automation: identify one code-first or contract-first authority, validate the normalized OpenAPI, repeat clean generation only for actual generated derivatives, diff compatibility at a fixed basis, verify every actual client or native adapter, and verify applicable backend success/auth/validation/business-error behavior. | Activates from REST alone, invents a generator or TypeScript/browser consumer, permits dual authorities, or treats static schema validation as live conformance. |
| Contract-first without generation: `Extend this hand-authored OpenAPI used by a native client; no generator or browser consumer exists.` | Baseline + Protocol automation: validate and diff the exact contract, verify the native adapter and applicable backend/consumer behavior, and mark generation/browser gates `Not applicable`. | Requires two generated outputs, a TypeScript client, or browser states that the repository does not own. |
...
| Protocol authority and live gate | When active, traces one authority through normalized OpenAPI to every actual generated client or native adapter, then proves applicable generation, compatibility, conformance, consumer, and clean-state gates or marks them `Not verified`. | Infers authority, invents generation or a frontend consumer, maintains dual sources, or claims live proof from static checks. |
