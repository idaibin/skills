...
| `Use ChatGPT now to review this fix branch, write review.md, then fix confirmed issues.` | Trigger `chatgpt-review` with explicit external-send authorization and require an `ops-browser` handoff for the low-level external UI actions. |
| `Send this review-package.md to ChatGPT for one review round.` | Trigger `chatgpt-review` for exactly one external round and require an `ops-browser` handoff on a verified route. |
...
| Review artifact | Stores only attributed external responses and local verification notes in `review.md`, including route, surface, account/workspace evidence, conversation, round, input/output mode, findings, and gaps. | Mixes the outbound package into `review.md` or produces unattributed review text. |
