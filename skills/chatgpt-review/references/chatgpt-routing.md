...
```text
~/.agents/config/chatgpt-review/defaults.yaml
```

Read that file after explicit per-request settings and before falling back to the generic ChatGPT URL. Surface and URL defaults may be reused after verification. A stored Current Chrome or standalone mode never replaces a current explicit request to use the user's browser. Do not store machine-specific defaults inside the installed skill package because package updates may overwrite them.
