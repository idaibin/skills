...
- Let `chatgpt-review` own the package, send authorization, surface, round count, conversation mapping, and response archive. Use `ops-browser` only for low-level browser actions and evidence on the route the review coordinator selected.
