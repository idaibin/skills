...
- App-native ChatGPT Project/Thread discovery, creation, messaging, response reads, lifecycle tracking, and model-evidence policy; use `ask-chatgpt`.
...
- For external ChatGPT collaboration, enter only when `ask-chatgpt` selected a browser webpage route. Accept its surface, authorization state, package path, round scope, route/capability, conversation mapping—or explicit policy to create exactly one first browser conversation—plus Chat/Work, model/reasoning preference, and ordered fallbacks. Perform only low-level webpage actions, verify rendered controls before submit, and return evidence. App-native Project/Thread operations remain entirely with `ask-chatgpt`. If the browser route, mapping, or selection cannot be revalidated, stop and return the break; do not switch sessions or unconfigured model modes, create an unrequested conversation, package repository content, decide that sending is authorized, add rounds, or write the response archive.
