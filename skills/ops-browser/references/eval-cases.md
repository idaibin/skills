...
| Browser/session preservation | Uses the requested browser or recorded session browser first, then the browser/tab containing required existing session or evidence, then the user's default browser such as Arc. | Probes or opens the default browser first when the evidence-bearing session is already in another browser. |
