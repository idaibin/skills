...
Use `<repo-root>/.codex/reviews/<review-id>-package.md` as the canonical durable outbound artifact unless the user names another path. Default `<review-id>` to `ask-<YYYYMMDD-HHmmss>` in local time. Keep its response log and related files directly in the same ignored `.codex/reviews/` parent with the same review-ID prefix; do not create a nested review directory. A compact review, research, architecture, UI, or image request that fits the inspected composer should use text without creating a file. Use file/pasted attachments only when size, structure, source assets, or multipart integrity requires them. If pasted content becomes an attachment, treat it as the single intended upload for that send action, verify the composer state, and do not paste or upload again unless the first attempt is removed or clearly failed. Classify attachment failures and retry eligibility with the shared browser-operation protocol; a chooser timeout alone is not proof that selection, upload, composer, target binding, and submission all remained unchanged. Never upload secrets, `.env`, private registry tokens, local keychains, browser profile data, unrelated dirty files, or unapproved source images.
...
Reject or mark `Not verified` if the tab is ambiguous, output is empty/truncated, or the response predates the submitted prompt. A provider generation or stop control that is still visible normally leaves terminal UI `Not verified`; when the response is already complete and attributable, accept it only through the shared protocol's bounded stable-response evidence: the same response container and conversation, a finite observation window and cadence fixed before sampling, at least two separated unchanged content-hash samples spanning that window, and no material response mutation. Capture it once, record the lingering control as a separate gap, and stop without resend. Do not use stable sampling to accept a partial, changing, differently attributed, or target-drifted response.
