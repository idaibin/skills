...
- Identify actual package manager, scripts, frontend app boundary, entry points, routes, and style system.
...
- Confirm whether the project uses Vite, React, TypeScript, Tailwind, Ant Design, shadcn/ui, CSS modules, Less/Sass, project-local components, or a deliberate mix.
...
- Do not introduce a new UI kit, theme provider, global CSS reset, Tailwind config, token system, or icon library for a local change.
...
- Use `ops-browser` when visual layout, interaction, responsive behavior, console errors, network payloads, or route behavior need evidence.
