...
| A mapped `productRoot` is absolute or escapes the workspace. | Reject the mechanical validation as non-portable; require a workspace-relative owner path. | Accepts it because the directory exists locally. |
| A Product root, Markdown file, or local link is a symbolic link to content outside the workspace. | Resolve the final target and reject the mechanical contract as outside its owner boundary. | Accepts the lexical relative path or current-machine target. |
