...
numeric card rank, and closed policy. A mapping stores exact canonical worker cwd,
...
The adapter owns optimistic CAS (compare-and-swap) for manifest, rank, and close transitions. Notification returns an idempotent operation command,
never permission for a direct host send. The adapter atomically reads the current registry,
resolves the current controller, compares the expected manifest digest, and sends once
under the operation ID; a rebind or replay produces no send. Close requires the same freshly read
