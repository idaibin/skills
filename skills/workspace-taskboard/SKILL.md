...
description: "Manage and visualize durable user-owned Codex tasks inside one verified local project root. Use when asked for 工作区任务看板、项目任务面板、管理项目内多个会话、发给已有会话、排队到这个会话、复用项目任务、新建项目任务、源码修改或测试默认委派、完成后通知当前会话、关闭任务、恢复或接管总控, show task status, queue work to an existing task, reuse or create a project worker, close a task, or invoke `$workspace-taskboard start`, `$workspace-taskboard status`, and `$workspace-taskboard resume CONTROL_ID`. Supports the project root and host-verified attached roots plus their realpath descendants; not global cross-project management or external provider sessions."
---
...
This Skill is the controller and in-conversation projection. It does not create a Web or
...
Show verified project/root, worker title and ID, exact cwd/relative label,
...
- Persist only controller/worker identity, verified roots, dependency edges, numeric
  rank, and closed policy. Query Git SHA, progress, tests, reports, and runtime evidence
  live; never copy them into the manifest.

## Coordinate And Notify

Workers proactively notify the current controller for completed, blocked,
decision-needed, basis-drift, ready-for-delivery, and delivered. Immediately before send,
read the manifest and verify the worker mapping, closed policy, current controller, and
project containment. Never notify a predecessor or a closed card. Use fixed-basis
