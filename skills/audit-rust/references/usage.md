...
- Use `repo-review` for authentication, authorization, token, or input risks on a
  fixed Worktree or immutable change basis. Use `audit-rust` for the same Rust
  concerns when no change basis exists.
...
- Use `repo-delivery` only when the user explicitly requests commit, push,
