...
| `Audit this known Rust module for authorization bypasses and token leakage with no diff basis.` | Prefer `audit-rust` with the relevant risk surfaces selected. |
| `Run a full vulnerability scan and validate exploitability across this Rust repository.` | Prefer an available host security-scan workflow; `audit-rust` may supply bounded Rust domain evidence but does not own scan coverage or PoC validation. |
...
| `Clippy reports dead code and a restriction lint in one feature build.` | Treat output as evidence; resolve public API, all applicable features/targets, cfg/macros/derives/build scripts/examples/FFI/downstream use and repository lint policy before findings. | Declares code dead or enables the entire restriction group from one configuration. |
...
| `Audit one Axum endpoint whose JSON extractor precedes a header extractor and whose CORS allows any origin.` | Ownership/errors; verify body-consumer ordering, rejection/error mapping, auth and middleware scope, then derive CORS from the real trust boundary. | Requires `State<Arc<_>>`, a universal timeout, or calls permissive CORS safe from an example alone. |
| `Audit a Tauri file command callable by one remote-enabled webview.` | Ownership/errors plus applicable target/security evidence; identify core/plugin versus custom command, verify `AppManifest::commands` and generated app-command permission before claiming capability invocation ACL, then separate enforced resource scope, Rust-owned user/domain authorization when applicable, CSP, caller, and real-client gap. | Assumes custom `invoke_handler` commands are capability-gated by default, trusts frontend types, duplicates platform ACL mechanically, treats CSP as command authorization, or ignores scope/domain policy. |
...
| Inspection snapshot | Common | records revision and relevant Worktree state for reproducibility without claiming diff attribution | audits an unrecorded moving tree or says the snapshot introduced an issue |
| Evidence-gated quality | Architecture/Baseline or selected owner | proves scope, Rust reachability, impact, owner/location and verification for duplicate, unused/dead, abstraction and coupling findings | reports a trait, clone, Arc/Mutex, file size, optional lint or one feature result by itself |
