...
2. Consume current `project-map`/repo-map Markdown or perform a targeted inventory of route/page entry, owning feature, analogous screens, UI primitives, layout/tokens, data/cache, forms/schema, state/store, tests, docs, and desktop adapter. Load `references/specification-authorities.md` when a selected profile depends on Product/UI contracts; resolve them by meaning rather than filename, consume Markdown authorities directly, and hand off only unresolved decisions required by the audit outcome. If a maintained non-LLM projection exists, inspect its declared consumer and drift lifecycle; otherwise do not require project YAML/JSON/schema/validator files. Identify derived maps with no consumer, repeated facts, and source-to-document drift as findings or `Not verified` gaps according to impact.
